...
Remove low-quality trajectories, retain 5 for the next step.
...
Our evaluation function consists of two components: (1) a rule-based reward, denoted as AutoEval(), and (2) a model-based reward, denoted as ExpertEval(). Importantly, neither component relies on ground-truth labels. To elaborate, AutoEval() is a rule-based mechanism designed to assess whether a generated trajectory meets minimal submission criteria. These criteria include, but are not limited to: ensuring the final patch file is non-empty, verifying that the trajectory contains a sufficient proportion of code-editing steps, and checking whether the trajectory length falls within a reasonable range. This procedure does not require access to ground-truth labels or test oracles.
...
We standardized all experimental conditions by using Claude-4-Sonnet as the base model for every method. Currently, our SE-Agent achieves 80.0% on SWE-bench Verified, significantly outperforming other approaches:
...
These results further validate the generality and effectiveness of our self-evolution framework.
...
| Model                | Metric           | SWE-Agent | SWE-Search (MCTS) | SE-Agent (Ours) |
|----------------------|---------------|-----------|-------------------|----------|
| Claude-3.7-Sonnet    | Times (min)      | 15.61     | 33.42             | 31.06    |
|                      | Resolved Rate (%)| 40.6      | 47.4              | 61.2     |
...
Compared to MCTS, Our experiments show that SE-Agent achieves significant improvements over MCTS-based methods in both search efficiency and effectiveness. SE-Agent introduces a trajectory-level evolutionary mechanism, enabling information exchange across different reasoning paths. This cross-trajectory interaction provides two key benefits: a. Breaking cognitive boundaries: Through competition and collaboration among trajectories, we overcome the limitations of individual reasoning paths. b. Knowledge transfer: Our recombination process facilitates the transfer of successful reasoning patterns across trajectories, while the revision step uses these recombined trajectories as targeted references for further improvement.
...
Moreover, we report Cost per Resolved Instance without any budget cap: SE-Agent: \$3.54, SWE-Search: \$4.08, SWE-Agent: \$4.66. This indicates that, despite a higher initial cost per run, SE-Agent is more cost-efficient in terms of successfully solving tasks.